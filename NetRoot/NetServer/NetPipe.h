...
namespace server_baby
{
	...
	private:
		...
		void MessageInorder(TreeNode<INT64, server_baby::NetUser*>* node);
		void UpdateInorder(TreeNode<INT64, server_baby::NetUser*>* node);
	...
	inline bool NetPipe::RegisterStub(NetStub* const stub)
	{
		if (!stub_)
		{
			stub_ = stub;
	...
	inline void NetPipe::MessageInorder(TreeNode<INT64, server_baby::NetUser*>* node)
	{
		if (node != userMap_.NIL_node_)
		{
			MessageInorder(node->left);
			DequeueJob(node->value);
			MessageInorder(node->right);
		}
	}

	inline void NetPipe::UpdateInorder(TreeNode<INT64, server_baby::NetUser*>* node)
	{
		if (node != userMap_.NIL_node_)
		{
			MessageInorder(node->left);
